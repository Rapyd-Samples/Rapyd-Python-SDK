...
class Duration1(Enum):
    ...
    def list():
        return list(map(lambda x: x.value, Duration1._member_map_.values()))


@JsonMap({})
class CreateCouponRequest1(BaseModel):
    def __init__(
        self,
        amount_off: float,
        currency: str,
        ...
        self.duration = self._enum_matching(duration, Duration1.list(), "duration")
...
class Duration2(Enum):
    FOREVER = "forever"
    REPEATING = "repeating"

    def list():
        return list(map(lambda x: x.value, Duration2._member_map_.values()))


@JsonMap({})
class CreateCouponRequest2(BaseModel):
    def __init__(
        self,
        duration: Duration2,
        percent_off: float,
        ...
        self.discount_valid_until = discount_valid_until
        self.discount_validity_in_months = discount_validity_in_months
        self.duration = self._enum_matching(duration, Duration2.list(), "duration")
        self.duration_in_months = duration_in_months
        self.id = id
        self.max_redemptions = max_redemptions
        self.metadata = metadata
...
class CreateCouponRequestGuard(OneOfBaseModel):
    class_list = {
        "CreateCouponRequest1": CreateCouponRequest1,
        "CreateCouponRequest2": CreateCouponRequest2,
    }


CreateCouponRequest = Union[CreateCouponRequest1, CreateCouponRequest2]
